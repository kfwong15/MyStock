...
def calculate_technical_indicators(df):
    ...
    rs = gain / loss
    df["RSI"] = 100 - (100 / (1 + rs))
    df["RSI"] = df["RSI"].fillna(50)  # 填充空值为中性值50
    ...
    <body>
        ...
        <div class="metrics">
            <div class="metric-box">
                <h3>今日价格</h3>
                <p>开盘: RM {data.get('open_price', 0):.3f}</p>
                <p>收盘: RM {data.get('close_price', 0):.3f}</p>
                <p>涨跌: {'📈' if data.get('change', 0) > 0 else '📉'} RM {data.get('change', 0):.3f} ({data.get('pct_change', 0):.2f}%)</p>
            </div>
            
            <div class="metric-box">
                <h3>技术指标</h3>
                <p>RSI: {rsi_value:.2f} {'(超买)' if rsi_value > 70 else '(超卖)' if rsi_value < 30 else ''}</p>
                <p>成交量变化: {volume_change:.2f}%</p>
                <p>布林带位置: {data.get('bollinger_position', '未知')}</p>
            </div>
        ...
        <div class="analysis">
            ...
            <div class="indicator {'critical' if '金叉' in data.get('trend_advice', '') or '死叉' in data.get('trend_advice', '') else ''}">
                <h3>交易信号</h3>
            ...
            <div class="indicator">
                <h3>关键价格水平</h3>
                <p>支撑位: RM {data.get('support_level', 0):.3f}</p>
                <p>阻力位: RM {data.get('resistance_level', 0):.3f}</p>
            </div>
        </div>
...
# 带重试机制的Yahoo Finance数据下载
def download_stock_data(stock, max_retries=3):
    for attempt in range(max_retries):
        try:
            print(f"🔍 获取 {stock} 数据 (尝试 {attempt+1}/{max_retries})...")
    ...
    print(f"⚠️ 无法获取 {stock} 数据，使用空DataFrame")
    return pd.DataFrame()
...
def main():
    ...
    print(f"📋 分析 {len(my_stocks)} 只股票: {', '.join(my_stocks)}")
    print(f"==================================================")
    ...
    for stock in my_stocks:
        try:
            ...
            if hist_df.empty:
                ...
                # 发送通知
                send_telegram_message(f"⚠️ 无法获取 {stock} 数据")
                continue
    ...
    print(f"==================================================")
    print(f"✅ 分析完成! 已处理 {len(my_stocks)} 只股票")
    print(f"===================================")
